...
def load_grades():
    ...
    local_path = get_data_path("grades.json")
    if os.path.exists(local_path):
        file_path = local_path
    else:
        file_path = resource_path("grades.json")
    try:
        ...
        with open(local_path, "w", encoding="utf-8") as f:
            json.dump({"subjects": subjects, "grades": {}}, f, ensure_ascii=False, indent=4)

def save_grades():
    file_path = get_data_path("grades.json")
    data = {
        "subjects": subjects,
        "grades": {
